...
<!-- #include file='resources/upload.asp' -->
<%
...
Dim temp_path: temp_path = "/images/temp_images/" & fields("temp_id") & "/"
Dim i: for i = 1 to ALLOWED_IMAGES
listing_image_paths(i) = Empty
Dim field_name: field_name = "image" & i
Dim listing_image_name: listing_image_name = i & ".jpg"
if uploader.Files.Exists(field_name) and mode <> "confirm" then
if uploader.Files(field_name).fileName <> Empty then
Set listing_images(i) = uploader.Files(field_name)
...
query = query & ("[" & field & "], ")
if field = "registered_with_rental_registry" then
values = values & (iif(fields(field) <> Empty, "true", "false") & ", ")
else
...
<body id='<%= mode %>'>
<!-- #include file='resources/categories.inc' -->
<div id='content'>
...
<!-- #include file='resources/listing.inc' -->
</div>
<div class='confirm'>
...
<%	end if %>
<!-- #include file='resources/listing_form.inc' -->
</div>
