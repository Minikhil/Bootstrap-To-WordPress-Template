<?php
$FM_VERS = "7.04";		// script version

/* ex:set ts=4 sw=4:
 * FormMail PHP script.  This script requires PHP 4 or later.
 * Copyright (c) 2001-2005 Root Software and Open Concepts Pty Ltd.
 * All rights reserved.
 *
 ...
 * Last Modified: RR 14:10 Mon 26 September 2005
 * QVCS Version: $Revision: 1.96 $
 *
 * Read This First
 * ~~~~~~~~~~~~~~~
 ...
 *		line 2714 in this file - replace "yourhost\.com" with your mail server's
 ...
 *
 * Removed parentheses which used to surround message numbers.
...
define("EMAIL_NAME","^[-a-z0-9.]+");	// the '^' is an important security feature!
    ...
    //
$TARGET_EMAIL = array(EMAIL_NAME."@buffalo\.edu$");
	//
...
$GEOIP_LIC = "";        // default - no GeoIP
    ...
    // exactly "0" (with the quotes) to be treated as empty.
    //
						...
						$s_friendly1 = "",$s_friendly2 = "")
{
	...
	switch ($s_oper)
	{
	case '&':		// bot fields must be present
		if ((isset($a_vars[$s_fld1]) && !IsFieldEmpty($a_vars[$s_fld1])) &&
			(isset($a_vars[$s_fld2]) && !IsFieldEmpty($a_vars[$s_fld2])))
			;		// OK
		else
		{
				//
	...
	case '^':		// either field but not both must be present
		$b_got1 = (isset($a_vars[$s_fld1]) && !IsFieldEmpty($a_vars[$s_fld1]));
		$b_got2 = (isset($a_vars[$s_fld2]) && !IsFieldEmpty($a_vars[$s_fld2]));
		if ($b_got1 || $b_got2)
		{
			if ($b_got1 && $b_got2)
			{
...
function CheckRequired($s_reqd,$a_vars,&$s_missing,&$a_missing_list)
{
    ...
    for ($ii = 0 ; $ii < count($a_list) ; $ii++)
    {
		...
		if (($i_span = strcspn($s_cond,REQUIREDOPS)) >= $i_len)
		{
			...
			if (!isset($a_vars[$s_fld]) || IsFieldEmpty($a_vars[$s_fld]))
			{
				$b_bad = true;
...
function RunTest($s_test,$a_vars)
{
	...
	elseif (($i_span = strcspn($s_test,$s_op_chars)) >= $i_len)
			...
			//
		$b_ok = isset($a_vars[$s_test]) && !IsFieldEmpty($a_vars[$s_test]);
	else
	{
			//
			// get first field name
...
function SubstituteValueWorker($a_matches,$s_repl,$b_html = true)
{
	...
	$s_name = $a_matches[1];
	$s_value = "";
	if (isset($aSubstituteValues[$s_name]) && !IsFieldEmpty($aSubstituteValues[$s_name]))
	{
...
function ProcessReturnToForm($s_url,$a_values)
{
	...
	if ($f_form === false)
		...
		//
		// read it all into one big string
		//
	$s_form_buf = "";
	while (!feof($f_form))
		$s_form_buf .= fread($f_form,1024);
...
function GetMultiValues($a_form_list,$i_form_index,$a_order = array(),
                                            ...
                                            $a_all_data = array())
{
    $a_ret_clean = $a_ret_raw = $a_ret_all = array();
    ...
    return (array($a_order,$a_ret_clean,$a_ret_raw,$a_ret_all));
}
...
{
    global  $bMultiForm,$SPECIAL_VALUES,$aSessionVars,$aServerVars;
...
function DetectSpecialsAttack($a_fields,&$s_attack,&$s_info)
{
            ...
            !empty($a_fields[$s_fld]))
    {
        ...
        if (preg_match("/^\b[-a-z0-9._%]+@[-.%_a-z0-9]+\.[a-z]{2,9}\b$/i",$s_data) === 1)
        {
                //
...
if (isset($aSessionVars["FormList"]))
{
    list($aFieldOrder,$aCleanedValues,
            $aRawDataValues,$aAllRawValues) = GetMultiValues(
...
if (!empty($SPECIAL_VALUES["fmcompute"]))
{
                            ...
                            &$a_values)
    {
        ...
        //
    function RegisterData($a_form_data)
    {
        global  $FMCalc;
